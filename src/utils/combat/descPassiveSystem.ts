...
function parseStatusGain(clause: string): DescPassiveAction | null {
  const statuses = ['Taunt', 'Offense Up', 'Defense Up', 'Speed Up', 'Retribution', 'Foresight', 'Stealth', 'Tenacity Up', 'Critical Damage Up', 'Critical Chance Up', 'Last Hope', 'Advantage'];
...
function parseWheneverRules(owner: CombatUnit, ability: Ability, requiresLeader: boolean): DescPassiveRule[] {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    else if (/critically hits?|critical hit/.test(cond)) { event = 'crit'; selfOnly = /^(an? )?(ally|allies)/.test(cond) ? false : true; }
