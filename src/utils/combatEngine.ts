...
import {
  onStackTrackerSpecial,
  onStackTrackerDamage,
  resetStackTrackerTurnFlags,
  refreshMuseumGuardian,
} from './combat/stackTrackerSystem';
...
export function getModifiedStats(unit: CombatUnit, teamUnits?: CombatUnit[]): {
  speed: number, offense: number, defense: number, critChance: number, critDamage: number,
  tenacity: number, potency: number, accuracy: number, evasion: number,
  defensePenetration: number, critAvoidance: number, healReceived: number, maxHpMult: number
} {
  ...
  unit.statuses.forEach(status => {
    ...
    if (def && def.statModifiers) {
      ...
      const scale = def.stackLimit > 1 ? count : 1;
      if (def.statModifiers.speed) speed *= Math.pow(def.statModifiers.speed, scale);
      if (def.statModifiers.offense) offense *= Math.pow(def.statModifiers.offense, scale);
      if (def.statModifiers.defense) defense *= Math.pow(def.statModifiers.defense, scale);
      
      if (def.statModifiers.speedAdd) speed += def.statModifiers.speedAdd * scale;
      ...
      if (def.statModifiers.potency) potency += def.statModifiers.potency * scale;
      
      if (def.statModifiers.critChance) critChance += def.statModifiers.critChance * scale;
      if (def.statModifiers.critDamage) critDamage += def.statModifiers.critDamage * scale;
      if (def.statModifiers.accuracy) accuracy += def.statModifiers.accuracy * scale;
      if (def.statModifiers.evasion) evasion += def.statModifiers.evasion * scale;
      if (def.statModifiers.defensePenetration) defensePenetration += def.statModifiers.defensePenetration * scale;
      if (def.statModifiers.critAvoidance) critAvoidance += def.statModifiers.critAvoidance * scale;
      if (def.statModifiers.healReceived) healReceived += def.statModifiers.healReceived * scale;
      if (def.statModifiers.maxHp) maxHpMult *= Math.pow(def.statModifiers.maxHp, scale);
    }
  });

  if (unit.statuses.some(s => s.name === 'Identity Disorder')) {
    critAvoidance += 0.25;
  }
  if (unit.dynamicState?.ignoreDefensePct) {
    defensePenetration = Math.max(defensePenetration, unit.dynamicState.ignoreDefensePct);
  }
  if (unit.dynamicState?.museumOffense) {
    offense = Math.round(offense * unit.dynamicState.museumOffense);
  }
  ...
  accuracy = Math.max(-0.9, Math.min(0.95, accuracy));
  evasion = Math.max(-0.9, Math.min(0.95, evasion));
  defensePenetration = Math.max(0, Math.min(0.9, defensePenetration));
  critAvoidance = Math.max(0, Math.min(0.95, critAvoidance));
  healReceived = Math.max(-0.9, healReceived);
  maxHpMult = Math.max(0.5, maxHpMult);

  return { speed, offense, defense, critChance, critDamage, tenacity, potency, accuracy, evasion, defensePenetration, critAvoidance, healReceived, maxHpMult };
}

/** Protection Up grant: count in (0,1] = fraction; (1,100] = percent; else 30%. */
export function grantProtectionUpAmount(target: CombatUnit, count: number): number {
  let pct = 0.30;
  if (count > 0 && count <= 1) pct = count;
  else if (count > 1 && count <= 100) pct = count / 100;
  return Math.round(target.maxProtection * pct);
}
...
export function applyStatus(state: CombatState, target: CombatUnit, name: string, duration: number, isDebuff: boolean, attacker: CombatUnit | null = null, count = 1, isDerived = false) {
  ...
  if (name === 'Info Broker') name = 'Information Broker';
  if (name === 'Crit Chance Up') name = 'Critical Chance Up';
  if (name === 'Expose') name = 'Exposed';
  if (name === 'Blocked') name = 'Ability Block';

  // Stash Protection Up percent count; grant after immunity checks succeed
  let protectionUpGrantCount: number | null = null;
  if (name === 'Protection Up') {
    protectionUpGrantCount = count;
    count = 1;
  }
  ...
  if (existingEff) {
    ...
    if (name === 'Protection Up' && protectionUpGrantCount != null) {
      const bonus = grantProtectionUpAmount(target, protectionUpGrantCount);
      if (bonus > 0) {
        target.protection += bonus;
        logBattleEvent(state, `🛡️ ${target.name} gains +${bonus} Protection from Protection Up!`, 'buff');
      }
    }
  ...
  if (!isDebuff && name === 'Artifact') {
    refreshMuseumGuardian(state, target);
  }
}
...
export function decrementStatusDurations(state: CombatState, unit: CombatUnit) {
  ...
  expired.forEach(s => {
    if (s.name === 'Explosive Charge') {
      const boom = Math.round(unit.maxHp * 0.20);
      unit.hp = Math.max(0, unit.hp - boom);
      logBattleEvent(state, `💣 Explosive Charge detonates on ${unit.name} for ${boom} damage!`, 'damage', unit.id, undefined, boom, false);
    } else if (s.name !== 'Thermal Detonator') { // already logged the boom
...
export function checkDefeat(state: CombatState, target: CombatUnit, attacker: CombatUnit | null) {
  if (target.hp <= 0 && target.activeInBattle) {
    ...
    // Reanimated: revive once with 20% Health
    if (target.statuses.some(s => s.name === 'Reanimated') || hasStatusFlag(target, 'reanimated')) {
      target.statuses = target.statuses.filter(s => s.name !== 'Reanimated');
      target.hp = Math.max(1, Math.round(target.maxHp * 0.20));
      target.activeInBattle = true;
      logBattleEvent(state, `🧬 Reanimated: ${target.name} revives with 20% Health!`, 'heal');
      return;
    }
...
export function executeCombatAction(
  ...
  if (ability.type === 'special' && assistDepth === 0 && counterDepth === 0) {
     ...
     onStackTrackerSpecial(state, attacker, ability);
  ...
  targetsToDamage.forEach(currentTarget => {
    ...
    if (ability.effects.some(e => e.includes('true_damage')) || ability.effects.some(e => e.includes('True Damage'))) {
      ...
      const hasGuaranteedEvade = (isSqueakyPayout || ignoreForesight)
        ? false
        : (hasStatusFlag(currentTarget, 'evade_next') || hasStatusFlag(currentTarget, 'evade_next_attack'));
      
      if (hasGuaranteedEvade && ability.type !== 'ultimate') {
        const evadeStatus = currentTarget.statuses.find(s => {
          const d = STATUS_DEFINITIONS[s.name];
          return d && (d.flags.includes('evade_next') || d.flags.includes('evade_next_attack') || d.flags.includes('consume_on_evade'));
        });
        const evadeName = evadeStatus?.name || 'Foresight';
        logBattleEvent(state, `💨 ${currentTarget.name} used ${evadeName} to evade the attack!`, 'info');
        currentTarget.statuses = currentTarget.statuses.filter(s => {
          const d = STATUS_DEFINITIONS[s.name];
          return !(d && (d.flags.includes('evade_next') || d.flags.includes('evade_next_attack') || d.flags.includes('consume_on_evade')));
        });
        baseDmg = 0;
        ...
        baseDmg = 0;
        onContractMiss(state, attacker);
      } else if (ability.type !== 'ultimate' && baseDmg > 0) {
        // Accuracy vs Evasion (Evasion Up / Accuracy Up/Down / Unconventional Tactics)
        const hitChance = Math.max(0.05, Math.min(0.95, 1 + (attStats.accuracy || 0) - (tarStats.evasion || 0)));
        if (Math.random() > hitChance) {
          logBattleEvent(state, `💨 ${currentTarget.name} evaded ${attacker.name}'s attack!`, 'info');
          baseDmg = 0;
          onContractMiss(state, attacker);
        }
      }
      ...
      if (baseDmg > 0) {
          ...
          // Veteran Orders / prevent_crit: cannot be critically hit
          if (hasStatusFlag(currentTarget, 'prevent_crit')) {
             critRoll = 1;
          }
          ...
          if (critRoll < Math.max(0, attStats.critChance - (tarStats.critAvoidance || 0))) {
            currentCrit = true;
          ...
          // Defense Penetration Up / Blaze Of Glory / Master Duelist / etc.
          if ((attStats.defensePenetration || 0) > 0) {
             targetDefense *= (1 - attStats.defensePenetration);
          }
          ...
          // Master Duelist / Veteran Commander ignore Taunt already handled by targeting; siege formation uses dynamicState
          if (attacker.statuses.some(s => s.name === 'Master Duelist' || s.name === 'Veteran Commander' || s.name === 'Siege Formation' || s.name === 'Museum Guardian')) {
            // defense pen already in getModifiedStats
          }
          ...
          // Entrenched / Veteran Orders: 25% reduced AoE damage
          if (targetsToDamage.length > 1 && hasStatusFlag(currentTarget, 'aoe_damage_reduction')) {
             currentFinalDamage = Math.round(currentFinalDamage * 0.75);
          }

          // Infested / Deathmark bonus damage
          if (currentTarget.statuses.some(s => s.name === 'Infested')) {
             currentFinalDamage = Math.round(currentFinalDamage * 1.25);
          }
          if (currentTarget.statuses.some(s => s.name === 'Deathmark')) {
             currentFinalDamage = Math.round(currentFinalDamage * 1.25);
          }
      ...
      if (currentTarget.statuses.some(s => s.name === 'Pursued')) {
        ...
        // Stagger: lose 100% Turn Meter on hit, then consume
        if (currentTarget.statuses.some(s => s.name === 'Stagger') || hasStatusFlag(currentTarget, 'stagger')) {
          currentTarget.turnMeter = 0;
          currentTarget.statuses = currentTarget.statuses.filter(s => s.name !== 'Stagger');
          logBattleEvent(state, `💫 Stagger: ${currentTarget.name} loses all Turn Meter!`, 'debuff');
        }

        // Drake Voss unique: whenever critically hit → Entrenched
        if (currentCrit && currentTarget.characterId === 'drake_voss') {
          applyStatus(state, currentTarget, 'Entrenched', 1, false, currentTarget);
          logBattleEvent(state, `🪖 Veteran Of Morvek: Drake Voss gains Entrenched after being critically hit!`, 'buff');
        }

        onStackTrackerDamage(state, attacker, currentTarget, ability);
      }
  ...
  if (assistDepth === 0 && counterDepth === 0) {
    ...
    if (finalDamage > 0) {
      ...
      if (isCrit) fireDescPassives(state, 'damage_taken', target, { statusName: 'crit_received' });
...
function parseAndApplyEffects(
  ...
  effects.forEach(effect => {
    ...
    const LONG_STATUS = new Set([
      ...
      'Wrath', 'Guarded Position', 'Veteran Commander', 'Duelist', 'Master Duelist',
      'Advance', 'Siege Formation', 'Museum Guardian', 'Artifact'
    ]);
    ...
    [
      ...
      'Accuracy Down', 'Evasion Down', 'Stagger', 'Armor Shred', 'Corruption', 'Debt', 'Hostage', 'Infested',
      'Information Broker', 'Info Broker', 'Explosive Charge', 'Foil', 'Tortured', 'Deathmark', 'Ambushed',
      'Predicted', 'Suppressed', 'Battlefield Corruption', 'Imperial Decree', 'Dossier', 'Analyze',
      'Investigation', 'Arrest Warrant'
    ].forEach(db => {
      ...
      if (effect.includes(db) || effect.includes(dbTag)) {
        ...
        // Canonicalize aliases
        const statusName = db === 'Expose' ? 'Exposed' : (db === 'Blocked' ? 'Ability Block' : (db === 'Info Broker' ? 'Information Broker' : db));
    ...
    [
      ...
      'Critical Chance Up', 'Crit Chance Up', 'Critical Damage Up', 'Inspired', 'Accuracy Up', 'Defense Penetration Up',
      'Evasion Up', 'Critical Avoidance Up',
      ...
      'Brotherly Love', 'Scum', 'Wrath', 'Guarded Position', 'Veteran Commander', 'Duelist', 'Master Duelist',
      'Advance', 'Siege Formation', 'Museum Guardian'
    ].forEach(bf => {
      ...
      if (effect.includes(bf) || effect.includes(bfTag)) {
        ...
        // Protection Up percent from kit text: "30% Protection Up" or "Protection Up (40%)"
        if (bf === 'Protection Up') {
          const pctMatch = ability.desc.match(/(\d+)%\s*Protection Up/i) || ability.desc.match(/Protection Up[^\d%]*\((\d+)%\)/i);
          if (pctMatch) count = parseInt(pctMatch[1], 10) / 100;
        }
        const statusName = bf === 'Crit Chance Up' ? 'Critical Chance Up' : bf;
        const duration = LONG_STATUS.has(statusName) || LONG_STATUS.has(bf) ? 99 : 2;
        ...
        recipients.forEach(a => applyStatus(state, a, statusName, duration, false, attacker, count));
      }
    });
    ...
    if (
      (effectLower.includes('heal') || (effectLower.includes('recover') && effectLower.includes('health'))) &&
      !effectLower.includes('immunity') &&
      effectLower !== 'health_up' &&
      !/^health up$/i.test(effect)
    ) {
      ...
      unitsToHeal.forEach(u => {
        if (hasStatusFlag(u, 'prevent_heal')) {
          ...
          const uStats = getModifiedStats(u, u.team === 'player' ? state.playerTeam : state.enemyTeam);
          const healBase = Math.round(u.maxHp * (uStats.maxHpMult || 1) * healPct);
          const healAmt = Math.round(healBase * (1 + (uStats.healReceived || 0)));
          u.hp = Math.min(u.maxHp, u.hp + healAmt);
    ...
    if (effectLower === 'evasion_up') applyStatus(state, attacker, 'Evasion Up', 2, false, attacker);
    if (effectLower === 'crit_avoidance_up') applyStatus(state, attacker, 'Critical Avoidance Up', 2, false, attacker);
    if (effectLower === 'accuracy_up' || effect === 'Accuracy Up') applyStatus(state, attacker, 'Accuracy Up', 2, false, attacker);
    if (effectLower === 'defense_penetration_up') applyStatus(state, attacker, 'Defense Penetration Up', 2, false, attacker);
