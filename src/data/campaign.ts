...
PLANET_CONFIGS.forEach((planetConfig, pIndex) => {
    ...
    for (let sIndex = 0; sIndex < totalSectors; sIndex++) {
        ...
        for (let nIndex = 0; nIndex < numNodes; nIndex++) {
            ...
            const designation = !isHard 
              ? `${planetConfig.name} ${sIndex + 1}-${nIndex + 1}`
              : `${planetConfig.name} ${sIndex + 1}-${nIndex - 10 + 1} Hard`;
            const nodeName = `${designation} - ${isBoss ? `${sectorName} Apex` : charReward ? `${charReward.name} Intercept` : 'Patrol'}`;
